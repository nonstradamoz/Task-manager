...
export default function CompletedTasksScreen() {
  ...
  const completedTasks = tasks.filter(t => t.completed && t.status !== 'archived').sort((a, b) => new Date(b.completedAt || b.updatedAt).getTime() - new Date(a.completedAt || a.updatedAt).getTime());
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      <View style={{ paddingTop: insets.top + 16, paddingHorizontal: 20, paddingBottom: 16, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
        ...
        <Text style={{ flex: 1, color: theme.colors.text, fontSize: 22, fontWeight: '800' }}>Completed Tasks</Text>
      </View>

      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 40 }}>
        {completedTasks.length === 0 ? (
          <EmptyState icon="checkmark-done-circle-outline" title="No Completed Tasks" subtitle="Tasks you complete will appear here." />
        ) : (
